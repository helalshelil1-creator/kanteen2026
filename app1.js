...
function applyLang(){
  ...
  Object.keys(bnavTexts).forEach(function(nav){
    ...
    if (!el) return;
    // اختار آخر span فقط (اللي فيه النص) وتجاهل الأيقونة والنقطة
...
/* ⭐ ktUpdateZoneBadge — محدّثة للعناصر الجديدة (#ktMegaZone / #ktMegaStatus / #ktZoneText) */
function ktUpdateZoneBadge(status, storeName, distance){
  // Update mega menu zone item
...
function route(){
  ...
  if (locBar){
    ...
    else locBar.classList.add('k-hidden'); // مخفي دايماً لحد ما المستخدم يضغط بحث
  }
...
function ktInit(){
  ...
  // ✅ Escape — يقفل الميجا + الشات + الدراور + المودال + البحث
